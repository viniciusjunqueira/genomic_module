...
!   Gibbs sampling
! IM  04/15/98-12/19/00
! IA  07/22/13 hash and matmul 
...
!     first entry, construct links
      ...
      do i=1,xx_ija%n
	    ...
	    if (hash_type==2) then 
	       ...
	       l=hashvr_old((/real(i,rh),real(k,rh)/),2,xx%x,xx%nel,3,0,xx%filled)
            endif
            if (l <1) then 
...
! xx must be of type sparse_ija and fully storred
  ...
  type(sparse_ija)::xx
  real (r8)::v(:,:),arhs(:),sol(:)
  integer::block,i,j,eq1,eq2,k,l,jj
  real (r8)::a,sum
  ...
  do i=xx%ia(n-offset1), xx%ia(n-offset1+1)-1
     ...
     a=xx%a(i)
     if (a.ne.0) then 
     ! dont use intrinsic function as this incurrs a significant overhead
     ! in memory allocation/deallocation
     ! arhs(eq1:eq2)=arhs(eq1:eq2)-a*matmul(v,sol((j-1)*block+1:j*block))
         jj=(j-1)*block
         do k=1,eq2-eq1+1
           sum=0
           do l=1,block
             sum=sum+v(k,l)*sol(jj+l)
           enddo
           arhs(k+eq1-1)=arhs(k+eq1-1)-a*sum
         enddo
      endif
  enddo
