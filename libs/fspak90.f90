...
!
  ...
  select case (operation(1:3))
       ...
       case('sol')            !solution
            ...
            deallocate(sol)
       case('m_inv_fact') ! sol= inv(factor)*rs 
           allocate(sol(ija%n))
            if (present(sol8) .and. present(rs8)) then
                sol=rs8
                call do_fact
                call run_fspak('s_inverse')
                sol8=sol
            elseif (present(sol4) .and. present(rs4)) then
                sol=rs4
                call do_fact
            ...
            deallocate(sol)
       case('m_fact') ! sol=factor*rs
           allocate(sol(ija%n))
	    if (present(sol8) .and. present(rs8)) then
                sol=rs8
                call do_fact
                call run_fspak('s_original')
                sol8=sol
            elseif (present(sol4) .and. present(rs4)) then
                sol=rs4
                call do_fact
   ...
   select case(op)
      ...
      case('solve')
            call fspak(50,ija%n,ija%ia,ija%ja,ija%a,sol,flag,6,99,memory,&
                    mem_needed,work,i,i,i,i,i,i,rank_o)
      case('m_inv_fact')
            call fspak(90,ija%n,ija%ia,ija%ja,ija%a,sol,flag,6,99,memory,&
	            mem_needed,work,i,i,i,i,i,i,rank_o)
      case('m_fact') 
            call fspak(100,ija%n,ija%ia,ija%ja,ija%a,sol,flag,6,99,memory,&
	            mem_needed,work,i,i,i,i,i,i,rank_o) 		     		    
      case('det')
