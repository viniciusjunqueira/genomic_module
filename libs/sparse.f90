...
type densem     !traditional dense square matrix
   integer :: n
   integer :: filled    ! for compatibility mainly
...
!------------------------------------------
 integer        ::     default_hash_size=5000,&
		       ...
		       hash_type=2            ! kind of hash function
                                              !  1 prime - 2 jenkins
 ...
 logical        ::     hash_size_power2=.false.,& ! force to be close to 2**n
                       default_zerosol=.true. ! zero solutions before 
                                              ! iterative solving	       
...
!=====================================================================
  ...
  interface	!external function
      function hashvr_old(dat,r,ind,m,n,mode,nr) 
      use kinds
      integer ::hashvr_old,r,mode,nr,m,n
      real (rh)::ind(:,:),dat(r)
      end function
  end interface
...
subroutine zero_densem(x,n,max_elem)
 ...
 integer :: n
 integer, optional :: max_elem !for compatibilty with zero sparse
 type (densem) :: x
!
...
!
!
! set defaults for new hash function
!
 if (hash_type==2) then 
     hash_size_power2=.true.
     hash_incr=2
     maxfill=0.80
 endif
 ...
 if (present(max_elem)) then
        x%nel=max_elem
        if (hash_size_power2) x%nel=sizepower2(max_elem) 
    else
        ! for matrix used previously apply old dimensions
        if (.not. associated(x%x)) then 
	   x%nel=default_hash_size
	   if (hash_size_power2) x%nel=sizepower2(default_hash_size)
        endif
 endif
!
 if (associated(x%x)) then
...
! get it from sparse_hashm structure 
 ...
 if (.not. upper_storage .or. j >=i) then
    if (hash_type == 2 ) then 
       ...
       full=hashvr_old((/real(i,rh),real(j,rh)/),2,x%x,x%nel,3,1,x%filled)
    endif
...
!     Matrix full, copy to a matrix hash_incr times larger
       ...
       call init(y)
       new_size = int(hash_incr*x%nel)
       
       call zerom(y,x%n,new_size,x%storage_type)
       do k=1,x%nel
          if (x%x(1,k) /= 0) then
       ...
       print'(a,i15,a,i15,a,f10.4)','hash matrix increased from ',x%nel,' to ',int(hash_incr*x%nel),&
                                  ' % filled: ',float(x%filled)/x%nel
       ! Move y to x
       call reset(x)
...
!
 ...
 if (.not. upper_storage .or. j >=i) then
    if (hash_type ==2 ) then 
       ...
       full=hashvr_old((/real(i,rh),real(j,rh)/),2,x%x,x%nel,3,0,x%filled)
    endif
    if (full <1) then
       a=0
     else
...
do i=1,x%nel
   if (x%x(1,i).ne.0) then
      ...
      if (int(x%x(1,i)) /= int(x%x(2,i)) .and. x%storage_type=='u') &
...
! converts symmetric hash matrix x into ija matrix y
 ...
 integer:: i,j,k,l,row(x%n),conv_type_o
 real (rh) :: zero=0
...
do k=1,x%nel
   if (x%x(1,k) /= 0) then
       call addm(x%x(3,k),int(x%x(1,k)),int(x%x(2,k)),y)
       if (int(x%x(1,k))<int(x%x(2,k)) ) &
          call addm(x%x(3,k),int(x%x(2,k)),int(x%x(1,k)),y)
   endif
enddo
end subroutine
...
do k=1,x%nel
   if (x%x(1,k) /= 0) then
       if (int(x%x(2,k))>=int(x%x(1,k)) ) &
          call addm(x%x(3,k),int(x%x(1,k)),int(x%x(2,k)),y)
   endif
enddo
end subroutine

integer function sizepower2(n) result (sp2)
! returns a number close to 2**i
! AL that is, find the closest, higher, power of two
! 5 -> 8 ; 8 -> 8
! http://stackoverflow.com/questions/4398711/round-to-the-nearest-power-of-two/4398995#4398995
integer :: n
sp2=2** (1+ int(log(real(n-1,r8))/log(2d0))) 
end function
end module
